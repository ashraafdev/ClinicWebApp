        ...
        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                    <div class="text-sm">
                        <div class="font-medium text-gray-700">{{ $consultation->patient->prenom }}
                            {{ $consultation->patient->nom }}</div>
                ...
                @if ((auth()->user()->getRoleNames()[0] == 'Assistant') || (auth()->user()->getRoleNames()[0] == 'Infirmier') || (auth()->user()->getRoleNames()[0] == 'Medecin' && $consultation->patient->medecin == auth()->user()->id))
                <td class="px-6 py-4">
                    <div class="flex justify-end gap-4">
                        @unlessrole('Infirmier')
                            <button
                                onclick='Livewire.emit("openModal", "consultation.edit-consultation", {{ json_encode(["consultation" => $consultation->id]) }})'>
                        ...
                        @hasrole('Infirmier')
                            <button
                                onclick='Livewire.emit("openModal", "consultation.mesure-upload", {{ json_encode(["consultation" => $consultation->id]) }})'>
                            ...
                            <button
                                onclick='Livewire.emit("openModal", "consultation.mesures-list", {{ json_encode(["consultation" => $consultation->id]) }})'>
